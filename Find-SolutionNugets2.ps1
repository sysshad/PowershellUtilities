...
.SYNOPSIS
    ...
    and optionally produces a wave-based upgrade order or circular dependency report.

.PARAMETER SearchPath
    Root folder to search. Defaults to the current directory.

.PARAMETER NuGetPrefixes
    Package-name prefixes to match. Default: 'Microsoft.Extensions', 'System.Text'.
...
.PARAMETER CheckCircularDependencies
    Runs a full DFS on every solution's ProjectReference graph and reports any
    circular chains, regardless of NuGet prefix matches.
    Solutions with no matching NuGets are still printed if a cycle is found.

.EXAMPLE
    .\Find-SolutionNugets.ps1 -SearchPath "C:\Source"

.EXAMPLE
    .\Find-SolutionNugets.ps1 -SearchPath "C:\Source" -CheckCircularDependencies

.EXAMPLE
    .\Find-SolutionNugets.ps1 -SearchPath "C:\Source" -ShowUpgradeOrder -CheckCircularDependencies
#>
[CmdletBinding()]
param (
...
# ---------------------------------------------------------------------------
# CIRCULAR DEPENDENCY DETECTION  (DFS with gray/black coloring)
#
# Colors: 0 = unvisited   1 = on current DFS stack (gray)   2 = done (black)
#
# A back edge (visiting a gray node) means a cycle exists.
# The cycle is extracted from the current stack, canonicalised by rotating to
# the lexicographically smallest node so duplicate detections are suppressed.
# ---------------------------------------------------------------------------
...
function Visit-DfsNode {
    ...
    foreach ($neighbor in @($Adj[$Node])) {
        ...
        if ($Color[$neighbor] -eq 1) {
            ...
            $startIdx  = $Stack.IndexOf($neighbor)
            $bodyCount = $Stack.Count - $startIdx   # number of nodes in the cycle body

            # Find the index of the lexicographically smallest node for canonicalisation
            $minIdx = $startIdx
            for ($i = $startIdx + 1; $i -lt $Stack.Count; $i++) {
                if ([string]::Compare($Stack[$i], $Stack[$minIdx], $true) -lt 0) {
                    $minIdx = $i
                }
            }

            # Build canonical key (rotation starting at smallest node)
            $canonParts = [System.Collections.Generic.List[string]]::new()
            for ($i = 0; $i -lt $bodyCount; $i++) {
                $canonParts.Add($Stack[$startIdx + (($minIdx - $startIdx + $i) % $bodyCount)])
            }
            $canonKey = [string]::Join('|', [string[]]$canonParts)

            if ($SeenKeys.Add($canonKey)) {
                # Store the display cycle: canonical rotation + closing node
                $displayCycle = [System.Collections.Generic.List[string]]::new()
                for ($i = 0; $i -lt $bodyCount; $i++) {
                    $displayCycle.Add($Stack[$startIdx + (($minIdx - $startIdx + $i) % $bodyCount)])
                }
                $displayCycle.Add($displayCycle[0])   # close the loop
                $Cycles.Add([string[]]$displayCycle)
            }
        }
        elseif ($Color[$neighbor] -eq 0) {
            Visit-DfsNode -Node $neighbor -Adj $Adj -Color $Color `
                          -Stack $Stack -Cycles $Cycles -SeenKeys $SeenKeys
        }
        # Color 2 (black) = already fully processed, skip
    }
...
function Find-CircularDependencies {
    ...
    foreach ($p in $Projects) {
        if (-not $p.ProjectPath) { continue }
        $norm    = [System.IO.Path]::GetFullPath($p.ProjectPath)
        $details = Get-ProjectDetails -ProjectPath $norm  # uses cache, no extra disk I/O
    ...
    foreach ($key in ($pathToName.Keys | Sort-Object)) {
        if ($color[$key] -eq 0) {
            Visit-DfsNode -Node $key -Adj $adj -Color $color `
                          -Stack $stack -Cycles $cycles -SeenKeys $seenKeys
        }
    }
...
if ($ShowUpgradeOrder)          { Write-Host "Upgrade order  : enabled" -ForegroundColor Cyan }
if ($CheckCircularDependencies) { Write-Host "Circular deps  : enabled" -ForegroundColor Cyan }
Write-Host ""
...
for ($si = 0; $si -lt $slnFiles.Count; $si++) {
    ...
    # Circular dependency scan runs on ALL projects regardless of NuGet matches.
    # Uses $script:DetailsCache populated above, so no extra disk I/O.
    $cycleResult = $null
...
foreach ($sol in $results) {
    ...
    # Skip solution entirely if there is nothing to show
    if ($affectedCount -eq 0 -and $cycleCount -eq 0) { continue }

    $printedCount++
    $projCount = $sol.Projects.Count

    # Build the PROJECTS summary line for the header
    ...
    # --- Solution header (printed exactly once) ----------------------------
    Write-Host ""
    ...
    Write-Host ("  PROJECTS  : " + ([string]::Join(', ', [string[]]$hdrParts)))   -ForegroundColor DarkGreen
    Write-Host $divOuter                                                           -ForegroundColor Cyan
    ...
    if ($affectedCount -gt 0) {
        ...
        Write-Host ""
        Write-Host "  (no projects matched NuGet filter)" -ForegroundColor Gray
    }
    ...
    if ($ShowUpgradeOrder -and $affectedCount -gt 0) {
        ...
        if ($order.Unresolvable.Count -gt 0) {
            ...
            if ($cycleAffected.Count -gt 0) {
                Write-Host ""
                Write-Host "  [!] WARNING: Circular ProjectReference chain detected in upgrade ordering:" -ForegroundColor Red
                foreach ($p in $cycleAffected) {
                    $label = if ($order.PathToName.ContainsKey($p)) { $order.PathToName[$p] } else { $p }
                    Write-Host "      - $label" -ForegroundColor Red
                }
                Write-Host "      Tip: run with -CheckCircularDependencies for the full cycle path." -ForegroundColor DarkGray
            }
        }
    }
    ...
    if ($CheckCircularDependencies) {
        Write-Host ""
        Write-Host $divInner -ForegroundColor Red
        Write-Host "  CIRCULAR DEPENDENCY CHECK" -ForegroundColor Red
        Write-Host $divInner -ForegroundColor Red

        if ($cycleCount -eq 0) {
            Write-Host "  No circular ProjectReference chains found." -ForegroundColor Green
        }
        else {
            $plural = if ($cycleCount -eq 1) { 'cycle' } else { 'cycles' }
            Write-Host "  $cycleCount $plural found  (fix these before attempting upgrades):" -ForegroundColor Red

            $cycleNum = 0
            foreach ($cycle in $sol.CycleResult.Cycles) {
                $cycleNum++
                $nodeCount = $cycle.Count - 1   # closing node is a repeat

                Write-Host ""
                Write-Host ("  [!] Cycle $cycleNum  ($nodeCount projects involved)") -ForegroundColor Red

                # Chain of names:  ProjectA -> ProjectB -> ProjectC -> ProjectA
                ...
                Write-Host ("      " + [string]::Join(' -> ', [string[]]$nameChain)) -ForegroundColor Yellow

                # Full paths for each unique node in the cycle
                Write-Host ""
                ...
                foreach ($nodePath in $cycle) {
                    ...
                    Write-Host ("      {0,-40} {1}" -f $n, $nodePath) -ForegroundColor DarkYellow
                }
            }
        }
    }
...
if ($skipped -gt 0) {
    Write-Host "  Solutions skipped : $skipped (no matching NuGets or circular deps)" -ForegroundColor DarkGray
}
if ($CheckCircularDependencies) {
    if ($totalCycles -eq 0) {
        Write-Host "  Circular deps     : none found across all solutions" -ForegroundColor Green
    }
    else {
        $plural = if ($totalCycles -eq 1) { 'cycle' } else { 'cycles' }
        Write-Host "  Circular deps     : $totalCycles $plural found -- fix before upgrading" -ForegroundColor Red
    }
}
Write-Host ""